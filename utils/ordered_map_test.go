...
func TestMap(t *testing.T) {
	...
	test.EqualsJSON(c, []string{"one", "two"}, m.Keys()) // orders preserved

	t.Logf("changing one won't change order")
	test.Assert(t, m.Put("one", -1).Found == true)
	test.Assert(t, m.Get("one").Found == true)
	test.Assert(t, m.Get("one").Value == -1)
	test.EqualsJSON(c, []int{-1, 2}, m.Values()) // orders preserved
	...
	test.EqualsJSON(c, []string{"two", "life", "everything"}, m.Keys()) // orders preserved

	test.Assert(t, m.Put("one", 1).Found == false)
	test.Assert(t, m.Get("one").Found == true)
	test.Assert(t, m.Get("one").Value == 1)
	test.EqualsJSON(c, []string{"two", "life", "everything", "one"}, m.Keys()) // added last after delete
}
